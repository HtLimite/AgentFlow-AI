...
Assert-True ($full.modules.provider_streaming -eq $true) "provider streaming module missing"
Assert-True ($full.modules.rerank -eq $true) "rerank module missing"
...
Assert-True ($runtime.capabilities.local_rerank -eq $true) "local rerank missing"
