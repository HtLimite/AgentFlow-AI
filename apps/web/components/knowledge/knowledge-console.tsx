...
interface KnowledgeAnswer {
  answer: string;
  citations: Array<{ document: string; content: string; score: number; keyword_hits?: number }>;
...
export function KnowledgeConsole() {
  ...
  async function query() {
    ...
    const citations = data.citations.length
      ? data.citations.map((item) => `- ${item.document} · score=${item.score} · hits=${item.keyword_hits ?? 0}: ${item.content}`).join("\n")
      : "无引用来源";
  ...
  async function deleteDocument(documentId: number) {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/knowledge-bases/${selectedId}/documents/${documentId}`, { method: "DELETE" });
      if (!response.ok) {
        const errorBody = await response.json().catch(() => null);
        throw new Error(errorBody?.detail || "删除失败");
      }
      await reload(selectedId);
      setAnswer(`已删除文档 #${documentId}，对应切片也已清理。`);
    } catch (error) {
    ...
    <div className="space-y-4">
      <Card className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">已接入数据库文档、切片、删除和低相关拒答。回答必须来自命中的引用片段，不再把无关 chunk 硬拼成答案。</p>
        </div>
      ...
      <Card>
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">删除文档会同时删除该文档产生的知识切片。</p>
          </div>
          ...
          {documents.length === 0 ? (
            ...
            documents.map((document) => (
              <div key={document.id} className="flex flex-col gap-3 rounded-xl bg-white/10 p-4 text-sm md:flex-row md:items-center md:justify-between">
                ...
                <Button onClick={() => void deleteDocument(document.id)} disabled={deletingId === document.id}>
