    ...
    <div className="space-y-4">
      <Card className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">追踪 Agent 每次工具调用的输入、输出、状态、耗时和 trace_id，支持筛选定位异常。</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          <div className="text-sm text-muted-foreground">调用总数</div>
      ...
      <Card>
        <div className="grid gap-3 md:grid-cols-3">
          <label className="block text-sm text-muted-foreground">
            <span className="mb-1 block text-xs text-muted-foreground/70">状态</span>
            <select className="w-full rounded-xl border border-border bg-panel/60 px-4 py-3" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>
          ...
          <label className="block text-sm text-muted-foreground">
            <span className="mb-1 block text-xs text-muted-foreground/70">工具</span>
            <select className="w-full rounded-xl border border-border bg-panel/60 px-4 py-3" value={toolFilter} onChange={(event) => setToolFilter(event.target.value)}>
          ...
          <label className="block text-sm text-muted-foreground">
            <span className="mb-1 block text-xs text-muted-foreground/70">关键字 / trace_id</span>
            <input className="w-full rounded-xl border border-border bg-panel/60 px-4 py-3" value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="搜索 trace、工具、输入、输出" />
          </label>
        </div>
      </Card>
      ...
      <div className="grid gap-4 xl:grid-cols-[420px_1fr]">
        <Card>
          ...
          <div className="mt-4 space-y-2">
            {loading ? (
              <div className="rounded-xl bg-surface/30 p-4 text-sm text-muted-foreground">加载中...</div>
            ) : filteredRecords.length === 0 ? (
              <div className="rounded-xl bg-surface/30 p-4 text-sm text-muted-foreground">当前筛选条件下暂无记录。</div>
            ) : (
              filteredRecords.map((item) => (
                <button
                  key={item.id}
                  onClick={() => setSelectedId(item.id)}
                  className={`w-full rounded-xl border p-4 text-left text-sm transition ${selected?.id === item.id ? "border-primary bg-primary-soft" : "border-border bg-surface/30 hover:border-border-strong"}`}
                >
                  <div className="flex items-center justify-between">
                    <span className="font-medium">#{item.id} · {item.tool_name}</span>
                    <span className={item.status === "success" ? "text-emerald-300" : "text-danger-foreground"}>{item.status}</span>
                  </div>
                  <div className="mt-2 text-xs text-muted-foreground">trace: {item.trace_id}</div>
                  <div className="mt-1 text-xs text-muted-foreground/70">{item.latency_ms}ms · {item.created_at}</div>
                </button>
              ))
            )}
          </div>
        ...
        <Card>
          ...
          <pre className="mt-4 max-h-[620px] overflow-auto whitespace-pre-wrap rounded-xl bg-panel/70 p-4 text-sm text-foreground">
