...
async def chat_completions(payload: ChatCompletionRequest, session: AsyncSession = Depends(get_db)) -> dict[str, object]:
    ...
    if provider is not None:
        try:
            ...
            await _record_observability(session, payload, provider.name, 0, 0, 0, status="failed", error_message=str(exc))
            raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail=f"Provider call failed: {exc}") from exc
        mode = "provider"
        warning = None
    else:
...
async def _record_observability(
    ...
    try:
        await persistent_observability_service.record(
            session,
            scenario="chat",
            model=payload.model or "local-fallback",
